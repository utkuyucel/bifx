...
def _backtest_overlay_strategy(merged_df: pd.DataFrame, config: BacktestConfig) -> dict:
    ...
    logger.info(f"XU100 Return: {total_return_xu100*100:.2f}%, Strategy Return: {total_return_strategy*100:.2f}%")
...
def run_backtest(fear_index_df: pd.DataFrame, xu100_df: pd.DataFrame, config: BacktestConfig = None) -> dict:
    if config is None:
